...
// One input reference with lifetime `'a` which must live at least as long as
// the function.
...
// Multiple elements with different lifetimes. In this case, it would be fine
// for both to have the same lifetime `'a`, but in more complex cases,
// different lifetimes may be required.
fn print_multi<'a>(x: &'a i32, y: &'a i32) {
  println!("`print_multi`: x is {}, y is {}", x, y);
}
fn print_multi_a<'a, 'b>(x: &'a i32, y: &'b i32) {
  println!("`print_multi`: x is {}, y is {}", x, y);
}
