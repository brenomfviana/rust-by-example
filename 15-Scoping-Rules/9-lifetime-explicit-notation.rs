...
// should be. The syntax for explicitly annotating a lifetime uses an
...
// `'a` and `'b`. These two lifetimes must both be at least as long as the
// function `print_refs`.
...
fn main() {
  ...
  // Any input which is borrowed must outlive the borrower. In other words, the
  // lifetime of `four` and `nine` must be longer than that of `print_refs`.

  failed_borrow();
  // `failed_borrow` contains no references to force `'a` to be longer than the
  // lifetime of the function, but `'a` is longer. Because the lifetime is
  // never constrained, it defaults to `'static`.
}
