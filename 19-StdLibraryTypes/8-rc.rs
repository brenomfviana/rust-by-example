...
// Rc's reference count becomes zero, which means there are no owners remained,
// both the Rc and the value are all dropped.
//
// Cloning an Rc never do a deep copy. Cloning creates just another pointer to
// the wrapped value, and increments the count.
...
fn main() {
  let rc_examples = "Rc examples".to_string();
  {
...

    {
      ...
      let mut rc_b: Rc<String> = Rc::clone(&rc_a);
      println!("Reference Count of rc_b: {}", Rc::strong_count(&rc_b));
      println!("Reference Count of rc_a: {}", Rc::strong_count(&rc_a));

      // Change rc_b value
      rc_b = Rc::new("Roberto Carlos".to_string());
